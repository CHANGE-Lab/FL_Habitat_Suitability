...
libraries("conflicted", "rgdal", "gdalUtils", "raster", "sp", "sf", "tmap", "dplyr", "lwgeom", "rgeos",
          "cleangeo", "tidyverse", "stars", "fasterize", "PNWColors", "spex", "igraph", 
          "spatialEco")
conflict_prefer("select", "dplyr")
conflict_prefer("filter", "dplyr")
conflict_prefer("xlim", "spex")
...
domain = st_read(dsn = paste0(temp_wd, "Constant_Hab_Shp.shp")) %>%
  st_transform(., my_crs)
domain = domain[domain$Value == 1,] # selecting only areas that had training habitat data
plot(domain)
...
lg_adult = rvc %>%
  group_by(REGION, STRAT, PRIMARY_SAMPLE_UNIT, STATION_NR, 
           LAT_DEGREES, LON_DEGREES, SPECIES_CD) %>% # group by both primary and second-stage sample units
...
# now combining the two data sources - RVC and MG - and clip
# adults
...
st_write(lg_adult_clip %>%
         ...
         dsn = paste0(fish_wd, "Presence_Absence/Adult_Gray_Snapper_PA.csv"), 
         append = FALSE)

st_write(lg_subadult_clip %>%
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]),
         dsn = paste0(fish_wd, "Presence_Absence/Subadult_Gray_Snapper_PA.csv"), 
         append = FALSE)

st_write(lg_juvenile_clip %>%
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]),
         dsn = paste0(fish_wd, "Presence_Absence/Juvenile_Gray_Snapper_PA.csv"), 
         append = FALSE)

# save presence-only data for MaxEnt modeling
...
st_write(lg_adult_clip %>%
         ...
         dsn = paste0(fish_wd, "Presence_Only/Adult_Gray_Snapper_PO.csv"), 
         append = FALSE)
...
st_write(lg_subadult_clip %>%
           ...
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]) %>%
           select(SPECIES_CD, LON_M, LAT_M), # maxent only needs species name, lon, lat (in that order)
         dsn = paste0(fish_wd, "Presence_Only/Subadult_Gray_Snapper_PO.csv"), 
         append = FALSE)
...
st_write(lg_juvenile_clip %>%
           ...
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]) %>%
           select(SPECIES_CD, LON_M, LAT_M), # maxent only needs species name, lon, lat (in that order)
         dsn = paste0(fish_wd, "Presence_Only/Juvenile_Gray_Snapper_PO.csv"), 
         append = FALSE)
...
st_write(hs_adult_clip %>%
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]),
         dsn = paste0(fish_wd, "Presence_Absence/Adult_Bluestriped_Grunt_PA.csv"), 
         append = FALSE)

st_write(hs_subadult_clip %>%
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]),
         dsn = paste0(fish_wd, "Presence_Absence/Subadult_Bluestriped_Grunt_PA.csv"), 
         append = FALSE)

st_write(hs_juvenile_clip %>%
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]),
         dsn = paste0(fish_wd, "Presence_Absence/Juvenile_Bluestriped_Grunt_PA.csv"), 
         append = FALSE)
...
st_write(hs_adult_clip %>%
           ...
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]) %>%
           select(SPECIES_CD, LON_M, LAT_M), # maxent only needs species name, lon, lat (in that order)
         dsn = paste0(fish_wd, "Presence_Only/Adult_Bluestriped_Grunt_PO.csv"), 
         append = FALSE)
...
st_write(hs_subadult_clip %>%
           ...
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]) %>%
           select(SPECIES_CD, LON_M, LAT_M), # maxent only needs species name, lon, lat (in that order)
         dsn = paste0(fish_wd, "Presence_Only/Subadult_Bluestriped_Grunt_PO.csv"), 
         append = FALSE)
...
st_write(hs_juvenile_clip %>%
           ...
           mutate(LON_M = sf::st_coordinates(.)[,1],
                  LAT_M = sf::st_coordinates(.)[,2]) %>%
           select(SPECIES_CD, LON_M, LAT_M), # maxent only needs species name, lon, lat (in that order)
         dsn = paste0(fish_wd, "Presence_Only/Juvenile_Bluestriped_Grunt_PO.csv"), 
         append = FALSE)
...
domain_kde = domain_kde + (1/10000) # add small constant value because bias grid can't have 0 in MaxEnt
...
win_bb_temp = bb_temp %>%
  ...
  mutate(ANN_MEAN_WIN_TEMP = mean(TEMP), SD_ANN_WIN_TEMP = sd(TEMP)) %>% # mean TEMP for each of the five years
  select(everything()) %>%
...
win_bb_sal = bb_sal %>%
  filter(SEASON == "winter") %>%
  group_by(SITE, X_COORD, Y_COORD, YEAR) %>% # annual means (a mean for each year)
  mutate(ANN_MEAN_WIN_SAL = mean(SAL), SD_ANN_WIN_SAL = sd(SAL)) %>% # mean SAL for each of the five years
  select(everything()) %>%
...
w_temp2
writeRaster(w_temp2, filename = paste0(spatial_wd, "Mean_win_Temp.asc"), format = "ascii", overwrite = T)
...
w_sal2
writeRaster(w_sal2, filename = paste0(spatial_wd, "Mean_Win_Sal.asc"), format = "ascii", overwrite = T)
...
w_do2
writeRaster(w_do2, filename = paste0(spatial_wd, "Mean_Win_DO.asc"), format = "ascii", overwrite = T)
...
s_temp2
writeRaster(s_temp2, filename = paste0(spatial_wd, "Mean_Sum_Temp.asc"), format = "ascii", overwrite = T)
...
s_sal2
writeRaster(s_sal2, filename = paste0(spatial_wd, "Mean_Sum_Sal.asc"), format = "ascii", overwrite = T)
...
s_do2
writeRaster(s_do2, filename = paste0(spatial_wd, "Mean_Sum_DO.asc"), format = "ascii", overwrite = T)
